 ...
 * - 注册时可设置超时，超时后 future 以 ReplyTimeoutException 异常完成并移除
 */
@Slf4j
@Component
@ConditionalOnProperty(prefix = "replyTracker.redis", name = "enabled", havingValue = "false", matchIfMissing = true)
public class InMemoryReplyTracker implements ReplyTracker {

    private final Map<String, CompletableFuture<CommonTopicResponse<?>>> futures = new ConcurrentHashMap<>();
    private final ScheduledExecutorService scheduler = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r, "reply-tracker-timeout");
        t.setDaemon(true);
        return t;
    });
    ...
    private void timeoutFuture(String tid, CompletableFuture<CommonTopicResponse<?>> f) {
        if (!f.isDone()) {
            ...
            f.completeExceptionally(new ReplyTimeoutException(tid));
